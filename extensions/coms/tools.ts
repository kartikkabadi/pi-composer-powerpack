...
/** Register coms tools with the Pi extension API. */
export function registerComsTools(pi: ExtensionAPI, deps: ComsToolsDeps): void {
		...
		async execute(_callId, params) {
			...
			const agents = collected.map((c, i) => {
				...
				return {
					...
					project: c.project,
					alive: pong != null,
					context_used_pct: pong ? pong.context_used_pct : -1,
					queue_depth: pong ? pong.queue_depth : -1,
					color: c.entry.color,
				};
			});
	...
	pi.registerTool({
		...
		label: "Coms Send",
		description:
			"Send a prompt to a peer agent. Returns the peer's response. " +
			"Use coms_list first to find available agents.",
		parameters: Type.Object({
			target: Type.String({ description: "Name of the target agent" }),
			prompt: Type.String({ description: "Prompt to send to the target agent" }),
			timeout_ms: Type.Optional(Type.Number({ description: "Response timeout in ms (default: 1800000)" })),
		}),
		async execute(_callId, params) {
			const identity = deps.getIdentity();
			if (!identity) {
				return { error: "coms not initialized — run /coms first" };
			}

			const entry = resolveTarget(params.target);
			if (!entry) {
				return { error: `agent "${params.target}" not found` };
			}
			...
			const timeout = params.timeout_ms ?? TIMEOUT_MS;
			try {
				const result = await Promise.race([
					sendEnvelope(entry.endpoint, envelope),
	...
	pi.registerTool({
		name: "coms_get",
		label: "Coms Get",
		description: "Get the next inbound message from the queue.",
		parameters: Type.Object({}),
		async execute() {
			const inbound = deps.getCurrentInbound();
			if (!inbound) {
				return { message: null };
			}
	...
	pi.registerTool({
		name: "coms_reply",
		label: "Coms Reply",
		description: "Reply to the current inbound message.",
		parameters: Type.Object({
			response: Type.Any({ description: "Response to send back" }),
		}),
		async execute(_callId, params) {
			const inbound = deps.getCurrentInbound();
			if (!inbound) {
			...
			// Reply is handled by the runtime
			return { success: true };
		},
	});
	...
	pi.registerTool({
		name: "coms_health",
		label: "Coms Health",
		description: "Check the health status of the coms system.",
		parameters: Type.Object({}),
		async execute() {
			const identity = deps.getIdentity();
			...
			let totalAgents = 0;
			for (const proj of projects) {
				totalAgents += pruneDeadEntries(proj).length;
			}

			return {
				healthy: identity !== null,
				identity: identity ? {
					name: identity.name,
				...
				pending_replies: deps.pendingReplies.size,
				timestamp: nowIso(),
			};
		},
	});
