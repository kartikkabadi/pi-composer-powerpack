 ...
 * @returns Path to the project agents directory
 */
 ...
 * @returns Path to the agent's registry file
 */
...
/**
 * Atomically write a registry entry.
 *
 * Writes to a .tmp file first, then renames to prevent corruption.
 *
 * @param entry - The registry entry to write
 * @param project - The project name
 * @returns The final path to the registry file
 */
...
/**
 * Read and parse all valid registry entries for a project.
 *
 * Skips malformed files silently.
 *
 * @param project - The project name
 * @returns Array of valid registry entries
 */
 ...
 *
 * @returns Array of all registry entries across all projects
 */
...
/**
 * Remove registry entries whose PID is no longer alive.
 *
 * @param project - The project name
 * @returns Array of surviving (alive) registry entries
 */
export function pruneDeadEntries(project: string): RegistryEntry[] {
	...
	for (const entry of entries) {
		try {
			...
			// ESRCH — process not alive, remove entry
...
/**
 * List all project names that have registry entries.
 *
...
/**
 * Resolve a target agent name to a registry entry.
 *
 * Searches across all projects for an agent matching the target name.
 *
 * @param target - The agent name to find
 * @returns The matching registry entry, or null if not found
 */
export function resolveTarget(target: string): RegistryEntry | null {
